...
// structured patch-applies and history rollbacks. Because every config write —
// raw apply, structured patch apply, and rollback — is serialized by applyMu,
// the persisted file must remain valid TOML throughout and no request may fail
// with a 5xx (Finding QA-1: concurrent patch + rollback safety).
func TestConfigApplyRollbackConcurrent(t *testing.T) {
	...
	rollback := func() *httptest.ResponseRecorder {
		...
		rr := httptest.NewRecorder()
		h.ServeHTTP(rr, httptest.NewRequest(http.MethodPost, "/api/history/rollback", bytes.NewReader(body)))
		return rr
	}
