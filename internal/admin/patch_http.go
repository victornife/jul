...
func (s *Server) applyPatchOps(w http.ResponseWriter, r *http.Request, params patchApplyParams) {
	...
	if result.OK && isTerminalApplyResult(result) {
		...
		s.recordAuditResourceSelector(r, params.auditAction, "config", patchBatchResourceIDs(execution.OperationSummaries), patchBatchSelectors(execution.OperationSummaries), "success", execution.summaryText())
