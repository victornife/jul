...
func TestAuditSinkTransitionKeepsRingAndGlobalIDs(t *testing.T) {
	...
	if got := readAuditIDs(t, aPath); len(got) != 1 || got[0] != 2 {
		t.Fatalf("A IDs=%v want [2]", got)
	}
	if got := readAuditIDs(t, bPath); len(got) != 1 || got[0] != 3 {
		t.Fatalf("B IDs=%v want [3]", got)
	}
}
...
func TestAuditSinkSamePathSharesPhysicalOwner(t *testing.T) {
	...
	if p2 == nil || p2.candidate == nil {
		t.Fatal("expected real rotation-policy transition")
	}
	if p2.candidate.owner != owner {
		t.Fatal("same-path transition opened a second physical owner")
	}
	p2.commit()
	p2.retire(context.Background())
	if a.currentSink.owner != owner {
		t.Fatal("physical owner changed after same-path handoff")
	}
	a.record(AuditEvent{Operation: "same-path", Result: "success"})
	if err := a.Close(); err != nil {
		t.Fatal(err)
	}
	if got := readAuditIDs(t, path); len(got) != 1 || got[0] != 1 {
		t.Fatalf("IDs=%v", got)
	}
}
...
func TestAuditSinkExactNoopDoesNotChurnGeneration(t *testing.T) {
	...
	if p2 != nil {
		t.Fatal("exact effective config should be a no-op")
	}
	if a.currentSink.id != gen || a.currentSink.owner != owner {
		t.Fatal("no-op churned sink generation")
	}
}
...
func TestAuditSinkDisabledNoop(t *testing.T) {
	...
	if p != nil {
		t.Fatal("disabled to disabled must not create a generation")
	}
}
...
func TestAuditSinkAbortPreservesExistingBytes(t *testing.T) {
	...
	if string(after) != string(before) {
		t.Fatalf("prepare/abort changed existing history: %q", after)
	}
}
...
func TestAuditSinkAbortRemovesCandidateOwnedEmptyArtifacts(t *testing.T) {
	...
	if _, err := os.Stat(path); err != nil {
		t.Fatalf("candidate placeholder: %v", err)
	}
	p.abort()
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("candidate file survived abort: %v", err)
	}
	if _, err := os.Stat(filepath.Join(d, "new")); !os.IsNotExist(err) {
		t.Fatalf("candidate dirs survived abort: %v", err)
	}
}
...
func TestAuditSinkConcurrentCutoverExactlyOneDestination(t *testing.T) {
	...
	if len(ids) != n {
		t.Fatalf("durable events=%d want %d", len(ids), n)
	}
	sort.Slice(ids, func(i, j int) bool { return ids[i] < ids[j] })
	for i, id := range ids {
		if id != int64(i+1) {
			t.Fatalf("durable ID set has gap/duplicate at %d: %v", i, ids)
		}
	}
}
...
func TestAuditSinkRotationCompatibilityAndRetention(t *testing.T) {
	...
	if len(ids) != 1 || ids[0] != 1 {
		t.Fatalf("active rotated sink IDs=%v", ids)
	}
}
...
func TestAuditSinkStatusRecoversOnSuccessfulWrite(t *testing.T) {
	...
	st := a.statusReport()
	if st == nil || !st.Healthy || st.LastFailureCategory != "" {
		t.Fatalf("status did not recover: %+v", st)
	}
	if st.WriteFailures != 2 {
		t.Fatalf("cumulative failures reset: %+v", st)
	}
}
