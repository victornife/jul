...
func TestAuditPrepareFailureDoesNotPoisonHealthyLiveSink(t *testing.T) {
	...
	if before == nil || !before.Healthy { t.Fatalf("live A not healthy: %+v", before) }
	...
	if err == nil { t.Fatal("candidate B unexpectedly prepared") }
...
func TestAuditDisableClearsActiveDegradationAndRingContinues(t *testing.T) {
	...
	if st := a.statusReport(); st != nil { t.Fatalf("disabled sink should not gate health: %+v", st) }
	a.record(AuditEvent{Operation: "ring-only", Result: "success"})
	if got := a.snapshot("", "", 0); len(got) != 1 || got[0].ID != 1 { t.Fatalf("ring continuity: %+v", got) }
}
...
func TestAdminAuditSinkPatchSparseAndExplicitDisable(t *testing.T) {
	...
	if !strings.Contains(detail, "audit_log_rotate_keep") { t.Fatalf("detail=%q", detail) }

	empty := ""
	if _, err := applyPatch(cfg, patchRequest{Op: "admin_audit_sink_set", AdminAuditSink: &adminAuditSinkPatch{File: &empty}}); err != nil { t.Fatal(err) }
	if cfg.Admin.AuditLogFile != "" { t.Fatalf("explicit empty path did not disable: %q", cfg.Admin.AuditLogFile) }
}
...
func TestReadyzAuditFailureIsBoundedAndDoesNotLeakPath(t *testing.T) {
	...
	if rr.Code != 503 { t.Fatalf("status=%d", rr.Code) }
	body := rr.Body.String()
	...
	if got["reason"] != "audit_sink" || got["status"] != "not ready" { t.Fatalf("readyz=%v", got) }
}

func TestResolveAuditSinkConfigCanonicalDefaultsAndValidation(t *testing.T) {
	cfg, err := resolveAuditSinkConfig("./audit.jsonl", 0, 0)
	if err != nil { t.Fatal(err) }
	if cfg.maxMB != 100 || cfg.keep != 14 || !cfg.enabled() { t.Fatalf("defaults=%+v", cfg) }
	if _, err := resolveAuditSinkConfig("audit.jsonl", -1, 14); err == nil { t.Fatal("negative max accepted") }
	if _, err := resolveAuditSinkConfig("audit.jsonl", 100, -1); err == nil { t.Fatal("negative keep accepted") }
	if cfg, err := resolveAuditSinkConfig("", 0, 0); err != nil || cfg.enabled() { t.Fatalf("empty path=%+v err=%v", cfg, err) }
}
