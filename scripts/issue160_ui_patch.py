...
repl='''    if (Object.keys(limits).length > 0) {
    ...
    if (Object.keys(auditSink).length > 0) next.push({ op: "admin_audit_sink_set", audit_sink: auditSink });
    return next;'''
...
section='''          <section className="rounded-lg border border-jul-border bg-jul-surface p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <label className="text-xs text-jul-muted">Rotate max MB <LifecycleBadge field={data.lifecycle.audit_log_rotate_max_mb} /><input type="number" min={0} step={1} value={auditRotateMaxMB} onChange={(event) => setAuditRotateMaxMB(event.target.value)} className="mt-1 w-full rounded-md border border-jul-border bg-jul-bg px-2 py-1.5 text-sm text-jul-text" /></label>
              <label className="text-xs text-jul-muted">Backups to keep <LifecycleBadge field={data.lifecycle.audit_log_rotate_keep} /><input type="number" min={0} step={1} value={auditRotateKeep} onChange={(event) => setAuditRotateKeep(event.target.value)} className="mt-1 w-full rounded-md border border-jul-border bg-jul-bg px-2 py-1.5 text-sm text-jul-text" /></label>
            </div>
            ...
            {auditPathChanging && !auditDisabling && <p className="mt-3 rounded-md border border-jul-warning/40 bg-jul-warning/10 p-2 text-xs text-jul-text">Existing audit files and backups at the previous path are not copied, moved, merged, or deleted. New durable events use the new destination only after the committed transition.</p>}
            {auditDisabling && <p className="mt-3 rounded-md border border-jul-warning/40 bg-jul-warning/10 p-2 text-xs text-jul-text">Durable persistence stops for new audit events after Publish. The in-memory audit ring and event IDs continue, and existing audit files/backups are retained.</p>}
            {auditRotationChanging && <p className="mt-3 rounded-md border border-jul-border p-2 text-xs text-jul-muted">The committed rotation policy governs the active sink. Preview never prunes backups; existing backups are only subject to normal retention during later active rotations.</p>}
            <p className="mt-3 text-xs text-jul-muted">Status: {data.audit_sink ? (data.audit_sink.healthy ? "Healthy" : data.audit_sink.active ? "Degraded" : "Configured, inactive") : "Disabled"}{data.audit_sink?.generation ? ` · generation ${data.audit_sink.generation}` : ""}{data.audit_sink?.last_failure_category ? ` · ${data.audit_sink.last_failure_category}` : ""}</p>
          </section>
