...
- Forwarding headers are ignored unless the immediate peer is explicitly trusted by the matched listener's [`[servers.client_address]`](docs/configuration.md#client-address-and-trusted-proxies) policy, which is empty by default. When a policy is configured, one canonical client address is derived per listen address before virtual-host routing ([ADR 0016](docs/adr/0016-inbound-identity-and-backend-peer-trust.md)); malformed, oversized or over-hop-limit input fails closed to the transport peer, and the peer itself always remains available as a separate fact. That one address is what CIDR authentication, IP rate limiting, the WAF's `REMOTE_ADDR`, access logs, outbound forwarding headers and the FastCGI environment all use — no consumer parses a forwarding header itself. Outbound `X-Forwarded-For` is rebuilt from Jul's own knowledge and never appends an inbound chain. Never trust client-supplied forwarding headers as security identity by default.
- The auxiliary `[egress]` allow-list and backend TLS identity solve different problems. Egress restricts selected configuration-driven fetch destinations; it does not authenticate reverse-proxy/gRPC backends. Backend peer identity is now expressible with [`backend_tls`](docs/upstreams.md#backend-tls) — private roots with an explicit `ca_mode`, a client certificate, an SNI override, a minimum version and explicit `peer_identities` — resolved into one immutable policy that transports consume without parsing configuration. `insecure_skip_verify` disables peer verification only (the connection stays encrypted); it is a `jul lint` **error**, warns once per backend at startup, and cannot be combined with `peer_identities` or a non-system `ca_mode`. The HTTP reverse proxy enforces the resolved policy today, including on WebSocket and streaming upgrades, with one transport per handler generation so a policy change is never served by a connection established under the previous trust, and with an explicit refusal to downgrade an `https` route to plaintext. Native gRPC and gRPC-JSON transcoding enforce it as well, reflection included, with the same refusal to downgrade to cleartext h2c, and active health probes verify a backend exactly as live traffic does — a backend is never reported healthy under weaker verification than the requests Jul will send it.
