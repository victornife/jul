...
| WASM response subscription | `plugins.subscription` in the request's phase record (subscription mode + ≤ 4 KiB request state) | request | plugin name within the request | — | pins the plugin of the generation that ran `handle_request`; no instance is held between phases (ADR 0020 §4, §11) | plugins |
...
| WASM response subscription | recorded when `handle_request` returns Continue after `subscribe_response` | — | discarded on Stop, or when the response never reaches the response point | request end | the response invocation acquires its own instance from the pinned generation's pool; that generation's runtime closes only after its in-flight requests (response phase included) drain | heap only, no runtime resource | `TestResponsePhasePinsRequestGeneration`, `TestV2ReloadUnderLoadAndChurnQuiesces` |
