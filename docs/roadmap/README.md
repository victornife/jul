...
| HP-07 | SSRF allow-list hardening | **Delivered:** optional, default-off egress allow-list (host/suffix/IP/CIDR) with typed decisions, hostname normalization, and subsystem-scoped guards. Applied consistently to every config-driven auxiliary client — JWKS, forward-auth, Consul/Kubernetes discovery, ACME/OCSP, and the WASM plugin `fetch` intersection — with `Proxy=nil` guarded transports, bounded `jul_egress_decisions_total` metrics, and Console Security visibility. Defense-in-depth: config is trusted per [SECURITY.md](../../SECURITY.md), so this bounds operator-error blast radius, not a request-driven hole. | Limits outbound reach even with a mistaken/compromised config | ✅ Delivered (P4-01 completes integration + diagnostics) | M |
