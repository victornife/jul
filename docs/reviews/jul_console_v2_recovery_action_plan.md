...
| TLS | ✅ Done (guided creation: static / ACME / mTLS); mTLS edit-existing shipped (Phase 4j: `MTLSEditor` + `server_set_client_auth` / `location_toggle_require_client_cert`) |
...
| 4i | L4 stream guided editor | P3 | ✅ Shipped — three structured ops manage `[[stream]]` L4 (TCP/UDP) listeners as a reviewed diff: `stream_add`, `stream_set` (targets the existing listener by its `protocol`/`listen` identity, so changing either re-keys it — surfaced as a remove + add in the diff), and `stream_remove`, each addressed by the normalized `protocol/listen` identity the validator dedups on. The new **Streams** panel (`GET /api/streams`) lists every declaration, offers a create/edit drawer (listen, protocol, default `proxy_pass`, SNI routing table, TLS passthrough, PROXY-protocol direction, connect/idle timeouts) with near-side validation mirroring the server (UDP rejects the TCP-only fields; a default backend or at least one SNI route is required), and removal. `diffStreams` annotates adds/removes/field changes with operational warnings. The panel and editor warn when the binary lacks the `stream` tag (declarations validate but a lean binary refuses to start with `[[stream]]`, mirroring the `wasmplugins` precedent) |
| 4j | mTLS edit-existing (guided) | P3 | ✅ Shipped — two structured ops close the last raw-only gap on an *existing* server's mutual TLS as a reviewed diff: `server_set_client_auth` (full-block edit of `mode` / `ca_file` / `crl_file` / `verify_san`, including disable→`none`, guarded so it refuses to disable while a route still requires a client cert) and `location_toggle_require_client_cert`. A new `GET /api/mtls` projection (`MTLSProjection`) carries every TLS-enabled server's `mode` / `ca_file` / `crl_file` / `verify_san` plus per-location `require_client_cert`, so the editor seeds every field; `require_client_cert` was also added to `LocationProjection` for the Routes detail. `diffMTLS` now emits a bind-time warning on any server-level mTLS change. **UI:** the TLS panel's **Mutual TLS** section (`MTLSEditor` drawer: mode select, CA bundle, CRL, SAN allow-list) plus a per-location `require_client_cert` toggle, also offered from the Routes detail quick-edits. **Guards:** `request`/`require` need a `ca_file`; enabling `require_client_cert` needs `client_auth` active. **UX:** a persistent **takes-effect-on-restart** banner — server-level `client_auth` is bind-time (listener rebind), *not* hot-reload; only per-location `require_client_cert` hot-reloads — the first editor to mutate a bind-time field |
