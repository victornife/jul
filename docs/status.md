...
| --- | --- | --- | --- |
| 2026-06-29 | 1.26 | **gRPC transcoding upstream pools + plugin fetch truncation + re-audit residuals:** gRPC-JSON transcoding now resolves `target` to an upstream pool (`[[upstreams]]` name or literal `host:port`), picks a backend per request for load-balancing and passive health-checking, and caches gRPC connections per backend address. The plugin `fetch` host function detects when a response exceeds `max_fetch_response` and exposes `last_fetch_truncated() -> i32` so guests can distinguish a capped body from a complete one; the SDK `Fetch()` wrapper pre-sizes its read buffer via `last_fetch_len` to eliminate a grow-and-retry loop. The plugin `fetch` SSRF guard now dials the *validated* IP (closing a DNS-rebinding window), and `fetch` gained `last_fetch_len`/`fetch_read`. The ABI golden pins full value types (not just arity), gRPC streaming rejects trailing tokens after `]` via decode-to-EOF, the Docker image builds with the `console` tag by default, and the deployment ACME example uses the nested `[servers.tls.acme]` shape. | [invoke.go](../internal/transcode/invoke.go), [fetch.go](../internal/plugins/fetch.go), [sdk.go](../examples/plugins/sdk/sdk.go), [abi-v1.golden](../testdata/plugins/abi-v1.golden), [Dockerfile](../Dockerfile) |
...
| 2026-06-29 | 1.23 | **ACME DNS-01 deprecated + multi-block consistency:** the unused `acme_dns` build tag is removed from CI/release/lint; `dns-01` and `dns_provider` are now documented as reserved for a future release and rejected with a clear message. Multiple ACME server blocks must agree on `email`, `ca`, `challenge`, `cache_dir`, and `ocsp_stapling`, since one autocert manager is shared process-wide — divergent values are rejected at config time instead of being silently dropped. | [tls-acme.md](tls-acme.md), [internal/config/validate.go](../internal/config/validate.go) |
| 2026-06-29 | 1.22 | **Plugin ABI compatibility suite:** the `jul-abi/v1` host-function surface is pinned by a golden snapshot (`TestABIV1Golden`) so a rename, retype, or removal can never merge silently — additive changes only — and the committed `testdata/plugins/*.wasm` guests run against the current host every CI run to catch backward-compat breaks. New `docs/abi.md` documents the additive-only-in-v1 versioning policy and how breaking changes ship as a new ABI id. | [abi.md](abi.md), [internal/plugins/abi_compat_test.go](../internal/plugins/abi_compat_test.go) |
| 2026-06-29 | 1.21 | **WASM plugin host hardening + fetch:** the `fetch` host function is now implemented — guarded outbound HTTP limited to `allowed_hosts`, with an SSRF guard that refuses loopback/private/link-local/CGNAT/multicast targets, redirect re-validation, per-call timeout, and a response cap. Request/response body buffering now fails the call on overflow instead of silently truncating, out-of-range guest statuses are replaced with `500`, and the per-plugin KV store enforces key-count and byte quotas. | [plugins.md](plugins.md#sandbox--capabilities), [internal/plugins/fetch.go](../internal/plugins/fetch.go) |
| 2026-06-29 | 1.20 | **gRPC transcoding hardening:** a unary JSON request body larger than `max_message_size` is now rejected with `413 Request Entity Too Large` instead of being silently truncated into a malformed message, and a JSON-array streaming request body is strictly framed — trailing tokens after the closing `]` are rejected with `400` so a request cannot smuggle silently dropped data past the parser. Single-backend dialing (first upstream server) is the documented transcoding MVP; use native passthrough for pool balancing. | [grpc-transcoding.md](grpc-transcoding.md#security--threat-notes), [internal/transcode/invoke.go](../internal/transcode/invoke.go) |
| 2026-06-29 | 1.19 | **Disk/response cache correctness:** `Vary` responses are now cached as distinct per-variant entries (base key + the request's varied-header values) with a small base-key pointer, so different variants of one URL (e.g. `Vary: Accept` JSON vs XML) coexist instead of evicting each other; `Vary: *` is no longer stored. Concurrent stale hits are deduplicated so a burst triggers exactly one background revalidation per variant. | [cache.md](cache.md#cache-key-and-vary), [internal/cache/cache.go](../internal/cache/cache.go) |
| 2026-06-28 | 1.18 | **Disk cache safety hardening:** the response cache's disk tier now writes entries atomically (same-dir temp file + fsync + rename) with owner-only mode `0o600`, and creates its directory `0o700` — a cached body is never world-readable and a crash mid-write can never leave a torn entry. Re-hydration is restricted to cache-owned files (64-char hex names); any foreign file in `disk_path` is ignored, never served, and never evicted, with a one-time `WARN` on startup. The memory→disk overflow hook now runs outside the memory-tier lock (victims collected under lock, persisted after release) so a slow disk cannot block in-memory reads/writes. New `docs/cache.md` documents the model, format, and safety properties. | [cache.md](cache.md), [internal/cache/diskstore.go](../internal/cache/diskstore.go) |
| 2026-06-28 | 1.17 | **UDP source-address churn soak:** new `udp-churn` soak scenario (`TestSoakUDPChurn`) drives sustained churn of short-lived UDP clients through a stream listener and asserts live sessions never exceed `max_udp_sessions`, the idle-reap/cap-evict paths fire, and sessions/goroutines/heap return to baseline after churn (no leak). `scripts/soak.sh` gains a `SOAK_SCENARIO` selector (`proxy`/`udp-churn`/`all`, default `all`) so the CI smoke and release soak gates now exercise both. Added an allocation-free `BenchmarkUDPAdmitAtCap` for the cap-admission hot path. | [stream-proxy.md](stream-proxy.md#soak-testing), [scripts/soak.sh](../scripts/soak.sh) |
