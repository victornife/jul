...
Detailed, per-feature engineering plans behind the [5-year roadmap](../roadmap/).
Year 1 and Year 2 specs are delivered/closed source-of-truth. **Year 3–5 specs are
concept horizons — not committed delivery schedules.** They capture target
problems, possible scope, architectural hypotheses, risks, non-goals, and the
evidence gate that would activate them. Implementation details must be
revalidated when a gate trips.
...
| 3 | Scale, fleet & ecosystem — **concept horizon** (clustering, multi-node config, RBAC/SSO, plugin registry) | [year-3.md](year-3.md) |
| 4 | AI-native + edge platform — **concept horizon** (semantic cache, tokenizer-aware routing, image optimization) | [year-4.md](year-4.md) |
| 5 | Global scale, mesh & cloud — **concept horizon** (geo-routing, service mesh, advanced edge security) | [year-5.md](year-5.md) |
