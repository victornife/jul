...
> Version 1.8 · Updated 2026-06-24
>
...
> **Y2-04 gRPC passthrough**, and **Y2-07 mTLS** are now **GA — soak pending** — published
...
- DoD: CRS loads+blocks OWASP test payloads; detect mode logs w/o blocking; per-location toggle; body inspection within limit; audit events emitted; off by default. **Shipped (2026-06-24, Beta):** `[waf]` global + `LocationConfig.WAF` override (`enabled`, `mode` block|detect, `block_status`, `directives_files[]`, `inline_rules`, `crs_enabled`, `paranoia` 0–4, `request_body_limit`, `response_body_check`). Built on Coraza v3 (`waf` tag) with the OWASP CRS bundle embedded via `coraza-coreruleset` (zero-setup when `crs_enabled = true`); user SecLang files + inline rules merge on top, and the engine mode (`SecRuleEngine On`/`DetectionOnly`) is applied last so config wins. The middleware wraps request+response phases (interruption → `block_status`, default 403; detect logs only), runs **after auth, before global compression**, and honours `request_body_limit`. Matched logged rules increment `jul_waf_events_total{action,rule}` and feed an error callback. The Console **Status** and **Security** panels report *Web application firewall (WAF)* with mode + active-location count (ADR 0004 DoD criterion 9). Build-tag gated like the stream proxy: a `waf.Check(cfg)` preflight fails fast if the binary lacks the `waf` tag but the config enables it. See [docs/waf.md](../waf.md) and [testdata/waf.toml](../../testdata/waf.toml). **Shipped delta:** WAF runs only when the `waf` build tag is present (lean builds reject WAF config rather than silently ignoring it); audit events are surfaced via the metric + structured log (the dedicated Y1-10 audit-sink wiring remains future work); CRS updates are bundled at build time (managed live updates are deferred to Y3).
...
- Secrets references (SEC-1, pulled earlier from Y5-06): introduce `env`/`file` secret refs + log redaction + a lint for literal secrets, used by ACME/JWT/forward-auth/mTLS (and the AI MVP key). Vault/KMS/SPIFFE remain later. **Shipped (2026-06-24, Beta):** string config fields accept `${env:NAME}`, `${file:/path}`, and `${secret:/path}` references, resolved at serve time across the whole config tree (`config.ExpandSecrets`, via reflection over every string field). Resolved values are registered with `internal/redact` and masked (`***`) in logs through a `redact.Writer` wrapper on the logger; `jul lint` flags literal `admin.token`, Consul, and Kubernetes tokens that are not references. The raw (unexpanded) config is what `TOMLSource.Load` returns, so the Console **Status**/**Security** panels can count unresolved references (`config.CountSecretRefs`) and report *Secret references* active. Implemented in core (no build tag). See [docs/secrets.md](../secrets.md). **Shipped delta:** `${env:}`/`${file:}`/`${secret:}` cover the local secret sources; `${secret:}` reads a file path today as the forward-compatible spelling for a future secret-manager backend (Vault/KMS/SPIFFE remain later).
...
| --- | --- | --- | --- | --- |
| 2026-06-24 | 1.8 | Marked **Y2-06 WAF** and **SEC-1 secrets references** shipped (**Beta**). WAF: `[waf]` global + per-location override on Coraza v3 (`waf` tag) with embedded OWASP CRS (paranoia 0–4), inline SecLang rules, block/detect engines (interruption → `block_status`), request/response body inspection within `request_body_limit`, `jul_waf_events_total{action,rule}`, a `waf.Check` build-tag preflight, and Console **Status**/**Security** surfaces. SEC-1: `${env:}`/`${file:}`/`${secret:}` references resolved across all string config at serve time (`config.ExpandSecrets`), automatic log redaction of resolved values (`internal/redact`), a `jul lint` literal-secret rule, and a Console secret-reference count (`config.CountSecretRefs`). Recorded shipped deltas for both. | All squad/quarter plans and the other feature specs; the three GA features stay **GA — soak pending** and Y2-08 GraphQL stays deferred. | [waf.md](../waf.md), [secrets.md](../secrets.md), [testdata/waf.toml](../../testdata/waf.toml); [ADR 0003](../adr/0003-maturity-and-ga.md), [ADR 0004](../adr/0004-console-ui-invariants.md) |
| 2026-06-21 | 1.6 | Relabeled the three GA features (Y2-01 transcoding, Y2-04 passthrough, Y2-07 mTLS) from **GA** to **GA — soak pending**, matching the honest label adopted fleet-wide once Y1-04 auth reached GA — the long-running soak test remains the open **post-GA gate** ([ADR 0005](../adr/0005-soak-post-ga-gate.md)). | All three features' runtime behaviour and every other Y2 spec; only the maturity-label wording changes. | [ga-push.md](../ga-push.md); [ADR 0005](../adr/0005-soak-post-ga-gate.md) |
