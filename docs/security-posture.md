...
- If remote access is required, configure [`[admin.tls]`](configuration.md#admin) with an
  ...
  insecure_transport` before authentication (ADR 0019 §28.1); only `/healthz`
  and `/readyz` are exempt.
