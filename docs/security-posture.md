...
- **Backend peer identity:** the `backend_tls` policy is defined, validated and resolved ([ADR 0016](adr/0016-inbound-identity-and-backend-peer-trust.md), [upstreams.md](upstreams.md#backend-tls)): trust roots with an explicit `ca_mode` enum, a client certificate, an SNI override that a discovery-returned address can never displace, a minimum version and explicit `peer_identities` checked after standard verification. `internal/backendtls` produces one immutable policy per backend and every consumer receives that type rather than the public fields. **The HTTP proxy, native gRPC, transcoding and health clients adopt it in the following tranches**; until then a configured block is validated, fingerprinted and surfaced, but those clients still use language defaults. An egress allow-list match is destination authorization, never proof of backend identity.
- **Backend clients not yet wired:** the policy exists, but HTTPS/gRPC upstreams and health probes still use the default transport verification until each integration lands. A private-CA backend is therefore still unreachable in practice, and `jul lint` warns when an `https` pool with an active HTTP probe has no policy.
