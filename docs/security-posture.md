...
- **Forwarded client identity:** the direct socket peer remains authoritative. The trust model is decided in [ADR 0016](adr/0016-inbound-identity-and-backend-peer-trust.md) — forwarding headers are honoured only when the immediate peer is explicitly trusted by the matched listener's `[servers.client_address]` policy, and one canonical client address is then shared by CIDR authentication, rate limiting, the WAF, logs and upstream forwarding. Until that policy is implemented and configured, behaviour is unchanged: headers are ignored and the peer is used. Never let arbitrary request headers redefine CIDR or rate-limit identity.
- **Backend peer identity:** outbound data-plane connections currently use language defaults. [ADR 0016](adr/0016-inbound-identity-and-backend-peer-trust.md) defines the `backend_tls` policy — trust roots, client certificate, SNI override, minimum version and explicit peer identities — shared by the HTTP proxy, native gRPC, transcoding and active health checks. An egress allow-list match is destination authorization, never proof of backend identity.
