...
python3 - "${container_ip}" "${CONTAINER_PORT}" "${proxy_port_file}" <<'PY' &
import socket
import sys
import threading
...
{
	echo "reference_observations_validated: ${FIXTURE_ID}"
	echo "equivalent_for_asserted_dimensions: ${FIXTURE_ID}/health-header"
	echo "expected_difference: ${FIXTURE_ID}/relative-redirect NGX_LOCATION_RETURN_ABSOLUTE_REDIRECT"
